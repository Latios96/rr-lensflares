...
layout(std430, binding = 4) buffer lensSystem { Lens lenses[]; };
...
Intersection intersectLens(inout Lens lens, vec3 position, vec3 direction) {
  ...
  normalize(intersectionPosition - vec3(0, 0, lens.center - lens.curvatureRadius)));
}
...
inout float maxRelativeDistanceToOpticalAxis, inout vec2 aperturePos) {
  ...
  max(maxRelativeDistanceToOpticalAxis, relativeDistanceToOpticalAxis);
...
void main() {
  ...
  intersectLens(lenses[firstReflectionIndex], tracedPosition, tracedDirection);
