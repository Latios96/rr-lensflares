...
typedef int ImGuiInputReadFlags; // -> enum ImGuiInputReadFlags_     // Flags:
                                 // for IsKeyPressedEx()
...
// Flags for IsKeyPressedEx(). In upcoming feature this will be used more (and
// IsKeyPressedEx() renamed) Don't mistake with ImGuiInputTextFlags! (for
// ImGui::InputText() function)
enum ImGuiInputReadFlags_ {
  // Flags for IsKeyPressedEx()
  ImGuiInputReadFlags_None = 0,
  ImGuiInputReadFlags_Repeat =
      1 << 0, // Return true on successive repeats. Default for legacy
              // IsKeyPressed(). NOT Default for legacy IsMouseClicked(). MUST
              // BE == 1.
  ImGuiInputReadFlags_RepeatRateDefault =
  ...
  ImGuiInputReadFlags_RepeatRateMask_ = ImGuiInputReadFlags_RepeatRateDefault |
                                        ImGuiInputReadFlags_RepeatRateNavMove |
...
struct ImGuiContext {
  ...
  ImGuiID NavActivateInputId;   // ~~ IsKeyPressed(ImGuiKey_Enter) ||
                              // IsKeyPressed(ImGuiKey_NavGamepadInput) ? NavId
                              // : 0; ImGuiActivateFlags_PreferInput will be set
                              // and NavActivateId will be 0.
...
IMGUI_API void GetKeyChordName(ImGuiModFlags mods, ImGuiKey key, char *out_buf,
                               int out_buf_size);
...
IMGUI_API ImVec2 GetKeyVector2d(ImGuiKey key_left, ImGuiKey key_right,
                                ImGuiKey key_up, ImGuiKey key_down);
IMGUI_API float GetNavTweakPressedAmount(ImGuiAxis axis);
IMGUI_API int CalcTypematicRepeatAmount(float t0, float t1, float repeat_delay,
                                        float repeat_rate);
IMGUI_API void GetTypematicRepeatRate(ImGuiInputReadFlags flags,
